...
from joint_state_planB import return_joint_state
...
class MoveGroupPythonInteface(object):
    ...
    def left_arm_move_type_judge(self):
        ...
        pose_goal = left_arm.get_current_pose().pose
        # 判断是否到达抓取工作空间/微调工作区间
        if (pose_goal.position.x >= 0.45 and pose_goal.position.x <= 0.60) and (pose_goal.position.y <= 0.30 and pose_goal.position.y >= 0) and (pose_goal.position.z <= 0.20 and pose_goal.position.z >= 0.0) :
            left_posecon = 1          
        else:
            left_posecon = 0
        return left_posecon
...
def main():
    ...
    T3 = 0
    T4 = 1
    global right_arm_state_time, left_arm_state_time
    right_arm_state_time = 1
    left_arm_state_time = 1
    # 输入回车,执行初始化程序
    ...
    while 1:
        ...
        if RightfingerT < -10 :
            if T1 == T2 :
                ...
                T1 = T2
                yumi.right_arm_go_to_joint_goal(right_arm_state_time)
                right_arm_state_time = right_arm_state_time + 1
        else :
            if T1 == T2 :
                T2 = not(T2)
            pass
        ...
        if LeftfingerT < -20 :
            if T3 == T4 :
                ...
                T3 = T4
                yumi.left_arm_go_to_joint_goal(left_arm_state_time)
                left_arm_state_time = left_arm_state_time + 1
        else :
            if T3 == T4 :
                T4 = not(T4)
            pass
