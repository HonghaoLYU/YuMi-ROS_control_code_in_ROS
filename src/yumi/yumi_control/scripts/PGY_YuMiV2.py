...
class MoveGroupPythonInteface(object):
    def __init__(self):
        ...
        # 订阅话题
        rospy.Subscriber('res_value', Float32, callback)
    ...
    def left_arm_go_to_joint_goal1(self):
        ...
        armnum = 'left_arm'
        # 设置末端关节目标值
        left_joint_goal[0] = left_joint_goal[0] + 0.1
    ...
    def right_arm_go_to_joint_goal1(self,jointchange):
        # 设置动作对象变量,此处为right_arm
        right_arm = self.right_arm
        # 获取当前末端执行器位置姿态
        right_joint_goal = right_arm.get_current_joint_values()
        armnum = 'right_arm'
        # 设置末端关节目标值
        right_joint_goal[0] = jointchange
        # print "End effector joint goal %s" % right_joint_goal
        # 规划并执行路径动作
        right_arm.go(right_joint_goal, wait=False)
        right_arm.clear_pose_targets()
...
def main():
    ...
    while 1:
        # if a == 2:
            print "============ Press `Enter` to  open right gripper 0.1rad..."
            raw_input()
            # 150Mpa 
            yumi.right_arm_go_to_joint_goal1(2.45)
            # 未充气
            # yumi.right_arm_go_to_joint_goal1(1.95)
            time.sleep(1.5)

        # if a == 3 :
        #     print "============ Press `Enter` to  open right gripper 0.2rad..."
        #     yumi.right_arm_go_to_joint_goal1(0.2)
        #     time.sleep(0.5)
        # if a == 4 :
        #     print "============ Press `Enter` to  open right gripper 0.3rad..."
        # #     raw_input()
        #     yumi.right_arm_go_to_joint_goal1(0.3)
        #     time.sleep(0.5)
